...
; Instructions if there is a problem adding the qlearningextension extension:
; inside the NetLogo directory, enter the 'app' directory
; then enter the 'extensions' directory, create a folder called qlearningextension
; and add the qlearningextension.jar file inside the created folder
...
; Instruções caso haja problema ao adicionar a extensão qlearningextension:
; dentro do diretório do NetLogo, entre no diretório 'app'
; depois entre no diretório 'extensions', crie uma pasta chamada qlearningextension
; e adicione o arquivo qlearningextension.jar dentro da pasta criada

extensions [ gis nw osmnlogo qlearningextension ]

breed [ GraphNodes GraphNode ]

undirected-link-breed [ GraphLinks GraphLink ]

breed [ Controladores Controlador ]

GraphNodes-own[ inlinks outlinks]
...
to setup
  ...
  ask GraphNodes with [ length inlinks > 1 ] [
    ...
    hatch-Controladores 1 [
      ...
      qlearningextension:state-def ["paradosNorte" "paradosOeste"]
      (qlearningextension:actions [ativarPlano1] [ativarPlano2])
      qlearningextension:reward [recompensa]
      qlearningextension:end-episode [isEndState] resetEpisode
      qlearningextension:action-selection "e-greedy" [0.7 0.995]
      set lr 0.1
      qlearningextension:learning-rate lr
      qlearningextension:discount-factor 0.3
...
to controlarFluxo
  ...
  if duracaoCiclo = 45 [
    set duracaoCiclo 0
    qlearningextension:decay-epsilon
    (qlearningextension:learn true)
...
to ativarPlano1
  set planoAtual "plano1"
end

to ativarPlano2
  set planoAtual "plano2"
end

to contaCarrosParados
  if-else planoAtual = "plano1" [
    if duracaoCiclo = 30 [
...
"" ""
PENS
"stopped" 1.0 0 -16777216 true "" "if ticks mod 45 = 0 [ plot count Vehicles with [ speed = 0 ] ]"
